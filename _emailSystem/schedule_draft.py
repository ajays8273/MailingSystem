...
number_of_mails = int(ldb.get_schedule_draft_count(email_id)[0])
#print number_of_mails , n , p


inbox_mails = ldb.get_schedule_drafts(email_id,n-10 ,n)
...
if(n <= number_of_mails ):
	print '<td><a href="redirect_spage_draft.py?page='+str(n)+'>next</a></td>'
if(p>=0):
	print '<td><a href="redirect_spage_draft.py?page='+str(p)+'>previous</a></td>'

print '</tr>'
print '<tr><td ><font size="4" color="red">To</td>\
			<td><font size="4" color="red">sub</td>\
			<td><font size="4" color="red">priority</td>\
			<td><font size="4" color="red">schedule_time</td>'

for row in inbox_mails :
			unique_id = str(row[0])+"@"+str(row[1])+"@"+str(row[6])
			print '<tr><td ><font size="4" color="blue">'+ row[1]+'</td>\
			<td><font size="4" color="blue"> <a href="show_sdraft.py?draft_id='+ unique_id+ '" >'+ row[2]+'</a> </td>\
			<td><font size="4" color="blue">'+ row[3]+'</td>\
			<td><font size="4" color="blue">'+ str(row[6])+'</td>\
			<td><font size="4" color="blue"> <a href="delete_sdraft.py?draft_id='+unique_id+'" >delete</a> </td>'
